...
const aws = processors.find((p) => p.id === "amazon-web-services" || p.id === "aws");
aws.name = "Amazon Web Services";
const hood = neighborhoodOf(aws, edges, processors, new Map());
expect("aws namers stay a count not a ring", hood.namers > 0 && hood.nodes.filter((n) => n.role === "namer").length === 0);
expect(
...
expect("no anonymous ring", !hood.nodes.some((n) => n.role === "namer") && edges.length > 0);
