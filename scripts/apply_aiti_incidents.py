#!/usr/bin/env python3
"""Merge curated first-party AI-incidents URLs into data.json and filled dossiers."""
...
def main() -> int:
    ...
    for row in data["companies"]:
        if row.get("slug") not in filed:
            row.pop("ai_incidents", None)
            inst = row.get("instruments")
            if isinstance(inst, dict):
                inst.pop("incidents", None)
    ...
    for slug, rec in recs.items():
        ...
        row["ai_incidents"] = dict(filed_rec)
        inst = row.setdefault("instruments", {})
        inst["incidents"] = dict(filed_rec)

        html_path = SITE / "c" / f"{slug}.html"
        html = html_path.read_text(encoding="utf-8")
        if url in html and ">AI incidents<" in html:
            continue
        row_html = (
            f"<tr><td>AI incidents</td><td>{official_a(url, host)}</td>"
